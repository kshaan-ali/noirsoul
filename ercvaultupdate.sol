...
contract VaultFactory is Ownable, ERC721Holder {
    ...
    uint256 public constant royaltyPercentage = 10; // 10% royalty
    ...
    function getVaultInfo(uint256 vaultId) public view returns (State, uint256) {
        Vault storage vault = vaults[vaultId];
        return (vault.sellingState, vault.offerTime);  // We can return other needed fields as well **like (... , .... , ...)
    }
...
contract FractionalToken is ERC20, ERC20Permit {
    ...
    function vaultApproval(address spender, uint256 value) public {
        ...
        (VaultFactory.State vaultState, uint256 offerTime) = vaultFactory.getVaultInfo(vaultId);
    ...
    _amount The number of tokens the buyer wants to purchase.
    Allows a buyer to purchase tokens from the available sell offers, starting with the lowest price.
    amount the buyer wants to purchase tokens with mormal numbering like 10 shares _amount will be 10. not 10e18 */
